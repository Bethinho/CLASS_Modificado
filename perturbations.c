...
int perturbations_derivs(double tau,
  ...
  double Ansatz_Q, Ansatz_deltaM, Ansatz_psiX ;

  Ansatz_Q = 3*(0.2)*y[pba->index_bi_rho_fld];
  Ansatz_deltaM = (a * Ansatz_Q * y[ppw->pv->index_pt_delta_cdm])/ppw->pvecback[pba->index_bg_rho_cdm];
  Ansatz_psiX = (a * Ansatz_Q * y[pv->index_pt_theta_cdm])/pvecback[pba->index_bg_rho_fld];
  /** ############################################################################### **/
  ...
  /** ##################################### MUDEI AQUI ################################ **/
      ...
      if (ppt->gauge == newtonian) {
        dy[pv->index_pt_delta_cdm] = -(y[pv->index_pt_theta_cdm] + metric_continuity - Ansatz_deltaM; /*  cdm density */

        dy[pv->index_pt_theta_cdm] = - a_prime_over_a*y[pv->index_pt_theta_cdm] -0.5*a2*ppw->pvecback[pba->index_bg_rho_cdm)*y[ppw->pv->index_pt_delta_cdm] + metric_euler; /* cdm velocity */
      }
  ...
  /** ###################################### MUDEI AQUI ###################################### **/
        ...
        /** - ----> fluid density */
         dy[pv->index_pt_delta_fld] = -(1+w_fld)*(y[pv->index_pt_theta_fld] -3.*(1 +w_fld)*a_prime_over_a*y[pv->index_pt_delta_fld] +metric_continuity; /** dp = wdx **/
      ...
      /**  dy[pv->index_pt_delta_fld] =
          ...
          -9.*(1+w_fld)*(cs2-ca2)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_theta_fld]/k2 - Ansatz2; **/

        /** - ----> fluid velocity */

        dy[pv->index_pt_theta_fld] = + (a * Ansatz_Q )/ppw->pvecback[pba->index_bg_rho_cdm]*y[pv->index_pt_theta_fld]
             + (1.+3*w_fld)*y[pv->index_pt_theta_fld] + Ansatz_psiX/(1.+w_fld) +metric_euler;
