...
/* ###################### MUDEI AQUI - so que não ################################### */

  /* cdm */
  if (pba->has_cdm == _TRUE_) {
    /** pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a,3); **/

    double numerador, constante_C, constante2,CC1;
    numerador = 0.2 + 1.0 + pvecback[pba->index_bg_rho_fld];
    constante2 = (0.2*(3.0 * pow(pba->H0,2) - pba->Omega0_cdm * pow(pba->H0,2)))/(0.2 + pvecback[pba->index_bg_rho_fld]);
    /** constante_C = 3.0 * pow(pba->H0,2)*(1 - (pvecback[pba->index_bg_rho_fld]*(3.0 * pow(pba->H0,2) - pba->Omega0_cdm * pow(pba->H0,2)))/(0.2 + pvecback[pba->index_bg_rho_fld]))/ pow(a,3); **/
    CC1 = (1 - (pvecback[pba->index_bg_rho_fld]*(3.0 * pow(pba->H0,2) - pba->Omega0_cdm * pow(pba->H0,2)))/(0.2 + pvecback[pba->index_bg_rho_fld]))/ pow(a,3);

    pvecback[pba->index_bg_rho_cdm] = 3.0 * pow(pba->H0,2)*CC1 + constante2 / pow(a,3*numerador);   /** Mudei **/
...
/* ##################################################################### */
  ...
  if (pba->has_fld == _TRUE_) {
   ...
   /* pvecback[pba->index_bg_rho_fld] = pvecback_B[pba->index_bi_rho_fld]; */
    ...
    double numerador2;
        numerador2 = 3*(0.2 + w_fld +1);
    pvecback[pba->index_bg_rho_fld] = 3*pba->Omega0_lambda * pow(pba->H0,2)/pow(a,numerador2);
...
/** ################################ Mudei aqui - ainda não! Mas vou olhar com cuidado esse trecho ################################## **/
  if (pba->has_fld == _TRUE_) {
...
/* ##################### TENHO QUE MUDAR AQUI - Densidade do fluido da energia escura ################### */
  if (pba->has_fld == _TRUE_) {
    /** - Compute fld density \f$ d\rho/dloga = -3 (1+w_{fld}(a)) \rho \f$ */
    dy[pba->index_bi_rho_fld] = -3.*(1.+pvecback[pba->index_bg_w_fld])*y[pba->index_bi_rho_fld] - 3*(0.2)*y[pba->index_bi_rho_fld];
  }
/* ###################################################################################################### */
  if (pba->has_scf == _TRUE_) {
